...
Estado::Estado(Ogre::SceneManager * mng, FMOD::System* sys, Ogre::RenderWindow* mWindow){
...
void::Estado::cargarAssetsAudio() {
	FMOD::Sound* sound;
	std::string aux, clave;
	FMOD_RESULT result;
	//std::string cabecera = "../sounds/";
	char* cabecera = "../sounds/";
...
void Estado::cargaAudio(std::string irPath) {
	// Sistema de audio
	cFx.resize(12);
	cMA.resize(6);
...
void Estado::reproduceFx(std::string fx, float x, float y, float z, float wet) {
...
void Estado::reproduceAmbM(std::string amb, float wet, bool fade) {
		bool cOcupied = false;
		int i = 0;
...
void Estado::paraAmb(int ch, bool fade) {
	if (fade) {
		unsigned long long parentclock;
